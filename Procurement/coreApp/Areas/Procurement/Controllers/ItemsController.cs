...
namespace coreApp.Areas.Procurement.Controllers
{
    ...
    public class ItemsController : ProcurementBaseController, IYearController
    {
                ...
                using(procurementDataContext context = new procurementDataContext())
                {
                    if (ModelState.IsValid)
                    {
                        tblItem item = new tblItem
                        {
                            ...
                            UnitId = model.UnitId,
                            Year = model.Year,
                            InDBM = !FixedSettings.ProcurementForPrivate && model.InDBM,
                            OrigPrice = model.OrigPrice,
                            _StockNo = model._StockNo,
                            MFO = model.MFO
                        };
            ...
            try
            {
                using (procurementDataContext context = new procurementDataContext())
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        item.InDBM = !FixedSettings.ProcurementForPrivate && model.InDBM;
                        item.OrigPrice = model.OrigPrice;
                        item._StockNo = model._StockNo;
                        item.MFO = model.MFO;
        ...
        [HttpPost]
        public ActionResult CopyItemSet(int currentYear, int newYear, bool overwrite = true, bool copyStockNos = true)
        {
            queryResult res = new queryResult { IsSuccessful = true, Data = null, Err = "", Remarks = "" };
