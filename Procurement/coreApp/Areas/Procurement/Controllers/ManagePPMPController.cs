...
namespace coreApp.Areas.Procurement.Controllers
{
    ...
    public class ManagePPMPController : ProcurementBaseController, IYearController
    {
        ...
        {            
            ...
            {
                var model = context.tblPPMPs
                    .Where(x => x.Year == Year).ToList()
                    .Where(x => !x.IsDepartmentPPMP && x.HasBeenSubmitted).ToList();
            ...
            try
            {
                using (procurementDataContext context = new procurementDataContext())
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        ppmp.ApprovedBy_UserId = access.user.Id;
                        ppmp.ApproveDate = DateTime.Now;
